...
def explanatory_analysis(charges_data_path='./data/charges_data.csv', personal_data_path='./data/personal_data.csv', plan_data_path='./data/plan_data.csv'):
    ...
    trimmed_mean = int(round(stats.trim_mean(col,proportiontocut=0.1),0))
    ################
    col = df_c['monthlyCharges'].dropna().sort_values()  # 정렬
    ...
    print(f"양쪽에서 제거할 개수: {cut_count}")

    # 실제 잘리는 값 확인
    lower_cut_values = col.iloc[:cut_count]
    upper_cut_values = col.iloc[-cut_count:]
    print("잘려나가는 하위 값들:")
    print(lower_cut_values)
    print("잘려나가는 상위 값들:")
    print(upper_cut_values)

    # trimmed mean 계산
    trimmed_values = col.iloc[cut_count:-cut_count]
    trimmed_mean = trimmed_values.mean()
    print(f"trimmed mean: {trimmed_mean}")
    ######################
    df_c['monthlyCharges'] = df_c['monthlyCharges'].fillna(trimmed_mean)
