...
enum Expr:
  ...
  def str: String = this match
    ...
    case EExit(expr) => s"exit(${expr.str})"
...
case class TypeEnv(
  ...
  def str: String =
    "TypeEnv {" + "\n" +
    (for ((name, ty) <- vars) yield s"  $name: ${ty.str}" + "\n").mkString +
    (for ((name, info) <- tys) yield s"  $name${info.str}" + "\n").mkString +
    "}"
}
  ...
  def str: String = this match
    ...
    case TIAdt(tns, vs) =>
      ...
      val vsStr = vs
        .map {
          case (vname, params) =>
            s"    case $vname(${params.map(_.str).mkString(", ")})"
        }
        .mkString("\n")
      s"$tnStr {\n$vsStr\n  }"
...
enum Type:
  ...
  // normalized type by renaming type variables to Tk form
  def norm: Type =
    def subst(ty: Type, map: Map[String, Int]): Type = ty match
      case IdT(name, tys) =>
        IdT(
          map.get(name).fold(name)(k => s"T$k"),
          tys.map(t => subst(t, map)),
        )
      case ArrowT(tns, pts, rty) =>
        val newMap = tns.foldLeft(map) {
          case (m, name) => m + (name -> m.size)
        }
        ArrowT(
          tns.map(name => s"T${newMap(name)}"),
          pts.map(t => subst(t, newMap)),
          subst(rty, newMap),
        )
      case _ => ty
    subst(this, Map.empty)
...
object Parser extends RegexParsers with PackratParsers {
  ...
  def EAnd(left: Expr, right: Expr): Expr = EIf(left, ENot(ENot(right)), F)
  def EOr(left: Expr, right: Expr): Expr = EIf(left, ENot(ENot(T)), right)
